...
public class DetailWidget extends DrawableHelper implements CharonActionCallback {
    ...
    public void init() {
        ...
        actionButton = this.parent.addButton(new ButtonWidget(this.rightPaneX + urlButtonWidths / 2 - cappedButtonWidth / 2,
                ...
                new TranslatableText("charon.install"), button -> {
        ...
        moreInfoButton = this.parent.addButton(new ButtonWidget(this.rightPaneX + urlButtonWidths + 4 + urlButtonWidths / 2 - cappedButtonWidth / 2,
                this.paneY + 36, Math.min(urlButtonWidths, 200), 20, new TranslatableText("charon.moreInfo"),
                button -> {
                }));
    ...
    private void showError() {
        MinecraftClient.getInstance().openScreen(new CharonErrorScreen((CharonInstallScreen) this.parent, this.selectedEntry.mod));
    }

    private void update() {
        this.actionButton.active = false;
        this.actionButton.setMessage(new TranslatableText("charon.updating"));
        this.parent.getService().updateMod(this, this.selectedEntry.mod);
    }

    private void remove() {
        this.actionButton.active = false;
        this.actionButton.setMessage(new TranslatableText("charon.removing"));
        this.parent.getService().removeMod(this, this.selectedEntry.mod);
    }

    private void install() {
        this.actionButton.active = false;
        this.actionButton.setMessage(new TranslatableText("charon.installing"));
    ...
    public void updateEntry() {
        ...
        if (selectedEntry == null) {
            ...
            actionButton.setMessage(new TranslatableText("charon.install"));
            return;
        }
        ...
        if (type == ProcessingType.ERRORED) {
            actionButton.active = true;
            actionButton.setMessage(new TranslatableText("charon.errored"));
            return;
        }
        if (this.parent.getService().isModInstalled(selectedEntry.mod)) {
            actionButton.active = true;
            if (this.parent.getService().isModOutdated(selectedEntry.mod)) {
                actionButton.setMessage(new TranslatableText("charon.update"));
                return;
            }
            if (selectedEntry.mod.id.equals("modmenu")) {
                actionButton.active = false;
                actionButton.setMessage(new TranslatableText("charon.modmenu.warning"));
                return;
            }
            actionButton.setMessage(new TranslatableText("charon.remove"));
            return;
        }
        actionButton.active = true;
        actionButton.setMessage(new TranslatableText("charon.install"));
